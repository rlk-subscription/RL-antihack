...
local function KickBanHacker()
    ...
    TriggerServerEvent('RL-antihack:server:drop')
end
...
RegisterNetEvent("RL-antihack:client:freezehacker", function()
    FreezePlayer()
    freezed = true
end)

RegisterNetEvent("RL-antihack:client:unfreezehacker", function()
    UnFreezePlayer()
    freezed = false
end)

RegisterNetEvent("RL-antihack:client:blowhacker", function()
    KickBanHacker()
end)

RegisterNetEvent("RL-antihack:client:killhacker", function()
    local id = PlayerPedId()
    SetEntityHealth(id, 0.0)
end)

RegisterNetEvent("RL-antihack:client:bancrashhacker", function()
    local id = PlayerPedId()
    local coords = GetEntityCoords(id)
    TriggerServerEvent('RL-antihack:server:ban')
    AddExplosion(vector3(coords.x, coords.y, coords.z), 5, 50.0, true, false, true)
    CrashHacker()
end)

RegisterNetEvent("RL-antihack:client:crashhacker", function()
    local id = PlayerPedId()
    local coords = GetEntityCoords(id)
    AddExplosion(vector3(coords.x, coords.y, coords.z), 5, 50.0, true, false, true)
    Wait(1000)
...
RegisterNetEvent('RL-antihack:client:antiHackMenu', function()
    local playerlist = {}
    QBCore.Functions.TriggerCallback('RL-antihack:server:GetOnlinePlayers', function(online)
        ...
        if menu then
            if not menu.id and not menu.mode then
                ...
                TriggerServerEvent('RL-antihack:server:action', tonumber(menu.id), tostring(menu.mode))
            end
        end
    end)
end)
...
RegisterNetEvent('qb-radialmenu:client:onRadialmenuOpen', function()
    QBCore.Functions.TriggerCallback("RL-antihack:server:isAdmin", function(isAdmin)
        if isAdmin then
            ...
            MenuItemId = exports['qb-radialmenu']:AddOption({
                ...
                icon = 'angry',
                type = 'client',
                event = 'RL-antihack:client:antiHackMenu',
                shouldClose = true
            }, MenuItemId)
        else
            MenuItemId = nil
